    ...
    public partial class frmSummaryOfStatusFinancialDocuments : XtraForm
    {
        ...
        List<StatusSummaryOfFinancialDocumentsVM> Obligations()
        {
            ...
            try
            {
                ...
                foreach (var i in unitOfWork.ObligationsRepo.Get(x => x.OfficeId == staticSettings.OfficeId && x.Year == staticSettings.Year && x.FT == staticSettings.FT, orderBy: x => x.OrderByDescending(m => m.Id)))
                {
        ...
        List<StatusSummaryOfFinancialDocumentsVM> PurchaseRequests()
        {
            ...
            try
            {
                UnitOfWork unitOfWork = new UnitOfWork();
                StaticSettings staticSettings = new StaticSettings();
                foreach (var i in unitOfWork.PurchaseRequestsRepo.Get(x => x.OfficeId == staticSettings.OfficeId && x.Year == staticSettings.Year && x.FT == staticSettings.FT, orderBy: x => x.OrderByDescending(m => m.Id)))
                {
                    var action = unitOfWork.DocumentActionsRepo
