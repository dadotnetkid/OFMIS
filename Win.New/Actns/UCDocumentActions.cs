    ...
    public partial class UCDocumentActions : DevExpress.XtraEditors.XtraUserControl, ILoad<DocumentActions>
    {
        ...
        public async void Init()
        {
            try
            {
                ...
                documentActions = unitOfWork.DocumentActionsRepo
                    .Fetch(x => x.RefId == refId && x.TableName == tableName).ToList();

                if (this.tableName == "PurchaseRequests")
                {
                    var po = unitOfWork.PurchaseOrdersRepo.Get(x => x.PRId == refId);
                    ...
                    foreach (var i in po)
                    {
                        foreach (var obr in i.Obligations)
                        {
                            ...
                            documentActions.AddRange(unitOfWork.DocumentActionsRepo.Get(x =>
